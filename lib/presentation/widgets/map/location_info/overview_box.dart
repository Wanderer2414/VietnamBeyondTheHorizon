...
class OverviewBox extends StatelessWidget {
  ...
  const OverviewBox({super.key, required this.location});

  bool _isOpenNow() {
    final now = DateTime.now();
    final format = DateFormat("HH:mm");
    final open = format.parse(location.openTime);
    final close = format.parse(location.closeTime);
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            Icon(location.typeIcon, size: 25, color: ColorPalette.primaryColor),
            SizedBox(width: 15),
            Expanded(
              child: Text(
                location.type,
                style: TextStyle(fontFamily: "Kay Pho Du", fontSize: 15),
              ),
            ),
          ],
        ),
        ...
        Row(
          children: [
            ...
            Expanded(
              child: Text(
                location.price +
                    ((location.price.toLowerCase() != "free") ? " VND" : ""),
                style: TextStyle(fontFamily: "Kay Pho Du", fontSize: 15),
              ),
            ),
          ],
